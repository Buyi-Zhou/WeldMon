...
DATA_FOLDER = 'data'  # Change this to your recording folder
...
def create_wave_file():
    ...
    filename = os.path.join(DATA_FOLDER, f'audio_{timestamp}.wav')
...
def record_audio(hat, num_channels):
    """Record audio using the original, working logic."""
    ...
    try:
        ...
        while True:
            ...
            print(f'\rSamples Read: {len(data)}', end='')
            stdout.flush()
            sleep(0.1)
        ...
        if initial_value < 200:
            ...
            for _ in range(30):
                ...
                if welder_current >= 200:
                    print("Welder_1 current went above 200, stopping monitoring and preparing to record...")

                    # Stop monitoring and wait for 3 seconds
                    sleep(3)

                    # Start recording
                    record_audio(hat, num_channels)

                    # Restart monitoring after recording is finished
                    break
                sleep(1)
            else:
                print("Welder_1 current did not go above 200, sleeping for 3 minutes.")
                sleep(3 * 60)

        else:
            print("Initial Welder_1 current is above 200, monitoring for 60 seconds...")
            previous_below_200 = False
            for _ in range(60):
                welder_current, _, _ = fetch_influxdb_data()
                if previous_below_200 and welder_current >= 200:
                    print("Welder_1 current went below 200 and then above 200, stopping monitoring and preparing to record...")
                ...
                print("Welder_1 current did not go below 200 and back up, sleeping for 3 minutes.")
                sleep(3 * 60)
...
def main():
    ...
    samples_per_channel = 2048  # Increase buffer size
    options = OptionFlags.CONTINUOUS
    scan_rate = 22050  # Lower the scan rate to reduce load
    hat = None
