...
# Ensure the script's working directory is the directory where the script is located
...
# Server details for file transfer
SERVER_IP = "192.168.0.246"
USERNAME = "adminkim"
PASSWORD = "SmartMFGatHK2024"
REMOTE_PATH = "~/maum/data/welding/weld_audio/"
...
def send_file_to_server(filename):
    """Send the recorded file to the remote server via SCP using paramiko."""
    try:
        if not os.path.exists(filename):
            raise FileNotFoundError(f"File {filename} does not exist on the Raspberry Pi.")
        ...
        sftp = ssh.open_sftp()
        sftp.put(filename, os.path.join(REMOTE_PATH, os.path.basename(filename)))
        sftp.close()
        ssh.close()

        print(f"File {filename} sent to server {SERVER_IP} successfully.")

        # Delete the file from the Raspberry Pi after successful transfer
        os.remove(filename)
        print(f"File {filename} deleted from Raspberry Pi.")
        ...
        print(f"Failed to send file to server: {e}")
...
def record_audio(hat, num_channels, tip_number, parts_welded):
    ...
    try:
        ...
        # Wait for 3 seconds before attempting to send the file
        sleep(3)
        ...
        if initial_value < 200:
            ...
            for _ in range(30):
                ...
                if welder_current >= 200:
                    print("Welder_1current went above 200, starting recording...")
                    record_audio(hat, num_channels, tip_number, parts_welded)
                    break
                sleep(1)
            else:
                print("Welder_1current did not go above 200, sleeping for 1 minute.")
                sleep(60)

        else:
            print("Initial Welder_1current is above 200, monitoring for 60 seconds...")
            previous_below_200 = False
            for _ in range(60):
                welder_current, _, _ = fetch_influxdb_data()
                if previous_below_200 and welder_current >= 200:
                    print("Welder_1current went below 200 and then above 200, starting recording...")
                    record_audio(hat, num_channels, tip_number, parts_welded)
                    break
                if welder_current < 200:
                ...
                print("Welder_1current did not go below 200 and back up, sleeping for 1 minute.")
                sleep(60)
